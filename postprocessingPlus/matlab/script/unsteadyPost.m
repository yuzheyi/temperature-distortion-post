...
%% 计算温升率
diff_temperature= diff(angleRange.highAverageTemperature');
diff_time = diff(planeData.time)
trans_temperature_rise_rate=diff_temperature./diff_time
% 找到第一个负数的索引
first_negative_index = find(trans_temperature_rise_rate < 0, 1);
% 提取第一个负数前的所有行，包括第一个负数
result = trans_temperature_rise_rate(1:first_negative_index-1);
temperature_rise_rate = mean(result)
...
dataTable = table(...
                    ...
                    'VariableNames', {'Time','Angle', 'AverageTemperature', ['HighAverageTemperature,rise_rate:' num2str(temperature_rise_rate) 'K/s']});
